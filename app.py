...
# 【固定定義】
USES = ["買い物", "キャンペーン", "ポイント還元", "ポイント利用", "チャージ", "失効", "発券・交換など", "貯", "運用推移"]
...
try:
    ...
    st.error("スプレッドシートの読み込みに失敗しました。共有設定が正しいか確認してください。")
    st.stop()
...
if menu == "通帳入力・履歴":
    ...
    if not display_df.empty:
        ...
        st.dataframe(display_df.sort_values("現金換算額", ascending=False), use_container_width=True)
